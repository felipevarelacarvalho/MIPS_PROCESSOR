...
def run_sim(asm_file=None):
	...
	if asm_file:
		...
		print('Use unix style paths like: MARsWork/Examples/addiSeq.asm')
		path = input('>').strip()
	...
	# open in write mode
	with open('temp/mars_dump.out','w') as f:
		subprocess.call(
			['java','-jar','MARsWork/Mars_CPRE381_v1.jar','nc',path],
			stdout=f
			)

	# Re-opens temp/mars_dump.out to check for errors
...
def check_mars_dump():
	...
	with open('temp/mars_dump.out') as f:
		for line in f:
